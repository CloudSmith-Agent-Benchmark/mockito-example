...
public class EmployeeManagerTest {
        ...
        @Test
        public void testPaymentTimeIsNotSetWhenPaymentFails() {
                Employee employee = spy(new Employee("1", 1000));
                when(employeeRepository.findAll())
                                .thenReturn(asList(employee));
                doThrow(new RuntimeException())
                ...
                // Verify payment timestamp remains null
                assertThat(employee.getLastPaymentTime()).isNull();
        }

        @Test
        public void testMultipleEmployeesPaymentTimes() throws InterruptedException {
                Employee employee1 = spy(new Employee("1", 1000));
                Employee employee2 = spy(new Employee("2", 2000));
                when(employeeRepository.findAll())
                                .thenReturn(asList(employee1, employee2));
                
                LocalDateTime beforePayments = LocalDateTime.now();
                Thread.sleep(10); // Small delay to ensure time difference
                
                assertThat(employeeManager.payEmployees()).isEqualTo(2);
                
                Thread.sleep(10); // Small delay to ensure time difference
                LocalDateTime afterPayments = LocalDateTime.now();
                
                // Verify both employees have payment timestamps within the expected range
                assertThat(employee1.getLastPaymentTime())
                        .isNotNull()
                        .isBetween(beforePayments, afterPayments);
                assertThat(employee2.getLastPaymentTime())
                        .isNotNull()
                        .isBetween(beforePayments, afterPayments);
                
                // Verify payment timestamps are in sequence
                assertThat(employee1.getLastPaymentTime())
                        .isBeforeOrEqualTo(employee2.getLastPaymentTime());
        }
